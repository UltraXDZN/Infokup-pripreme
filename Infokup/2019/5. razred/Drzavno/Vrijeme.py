...
rastavljeno_vrijeme = list(zadano_vrijeme)
print(rastavljeno_vrijeme)

opcije_vremena = []

for i in range(1, len(zadano_vrijeme)+1):
    if int(zadano_vrijeme[:i]) <= 23 and int(zadano_vrijeme[i:]) <= 59:
        if "0" not in zadano_vrijeme[-2::-i]:
            opcije_vremena.append((zadano_vrijeme[1:i], zadano_vrijeme[i:]))
